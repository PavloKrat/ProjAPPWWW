...
function displayEditCategoryForm($conn, $id) {
    // Получение текущей информации о категории
    $stmt = $conn->prepare("SELECT matka, nazwa FROM kategorie WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if (!$kategoria) {
        return "<p>Nie znaleziono kategorii o ID: " . htmlspecialchars($id) . "</p>";
    }
    ...
    // Формирование формы для редактирования
    $wynik = "
    <h2>Edytuj kategorię</h2>
    <form method='post'>
        <label for='matka'>Matka (ID):</label><br>
        <input type='number' name='matka' id='matka' value='" . htmlspecialchars($kategoria['matka']) . "'><br><br>
...
function displayAddProductForm($conn) {
    if (isset($_POST['dodaj_produkt'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO produkty (tytul, opis, cena_netto, podatek_vat, ilosc_sztuk, kategoria_id, zdjecie) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssdidss", $tytul, $opis, $cena_netto, $podatek_vat, $ilosc_sztuk, $kategoria_id, $zdjecie);
    ...
    $wynik = "
        ...
        <form method='post'>
            ...
            <input type='number' name='cena_netto' id='cena_netto' step='0.01' required><br><br>
            ...
            <input type='text' name='zdjecie' id='zdjecie' placeholder='https://example.com/image.jpg' required><br><br>
...
function displayEditProductForm($conn, $id) {
    ...
    if (!$produkt) {
        return "<p>Nie znaleziono produktu o ID: " . htmlspecialchars($id) . "</p>";
    }
    ...
    if (isset($_POST['edytuj_produkt'])) {
        ...
        $stmt = $conn->prepare("UPDATE produkty SET tytul = ?, opis = ?, cena_netto = ?, podatek_vat = ?, ilosc_sztuk = ?, kategoria_id = ?, zdjecie = ? WHERE id = ?");
        $stmt->bind_param("ssdidisi", $tytul, $opis, $cena_netto, $podatek_vat, $ilosc_sztuk, $kategoria_id, $zdjecie, $id);
    ...
    $kategorie = fetchCategoriesForDropdown($conn, $produkt['kategoria_id']);
    ...
    <form method='post'>
        <label for='tytul'>Tytuł:</label><br>
        <input type='text' name='tytul' id='tytul' value='" . htmlspecialchars($produkt['tytul']) . "' required><br><br>

        <label for='opis'>Opis:</label><br>
        <textarea name='opis' id='opis' rows='5' cols='50'>" . htmlspecialchars($produkt['opis']) . "</textarea><br><br>

        <label for='cena_netto'>Cena netto:</label><br>
        <input type='number' name='cena_netto' id='cena_netto' step='0.01' value='" . $produkt['cena_netto'] . "' required><br><br>
